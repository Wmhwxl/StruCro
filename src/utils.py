...
def wavelet_transform(x, wavelet='db1', level=1):
    x = x.unsqueeze(1)
    coeffs = pywt.wavedec(x.cpu().detach().numpy(), wavelet, level=level)
    coeffs[1:] = (np.zeros_like(v) for v in coeffs[1:])
    x_transformed = pywt.waverec(coeffs, wavelet)
    x_transformed = x_transformed.squeeze(1)
    return torch.tensor(x_transformed, device=x.device)

def mask_data(x, mask_ratio=0.15, seed=42):
